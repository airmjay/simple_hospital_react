...
export default function AppointmentTable({ props }) {
    ...
    const [diagnose, setDiagnose] = useState(row?.diagnose)
    ...
    return (
        <StyledTableRow key={row.id}>
            ...
            <StyledTableCell align="right">{!isEditing ? <span> {row.diagnose}</span> :
                (<div style={{ display: 'flex', alignItems: 'center' }}>

                    <input type='text' value={diagnose} onChange={(e) => setDiagnose(e.target.value)} />
                              ...
                              current.map((currentApp)=>
                                    {
                                        if(currentApp.id == row.id)
                                            {
                                                return {...currentApp, doctor , age, diagnose, name, appointment}
                                            }
